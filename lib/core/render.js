...
var debug = require('debug')('k+nav:debug');
...
function render (req, res, view, ext) {
	...
	var nav_user = keystone.nav;
	if (keystone.get('admin access control') && req.user){
		nav_user = {
			sections: keystone.nav.by.role[req.user.role],
			by: {
				list: keystone.nav.by.list
			}
		};
	}
	...
	var locals = {
		...
		nav: nav_user,
		uiOptions: keystone.get('admin uiOptions'),
		messages: _.some(flashMessages, function (msgs) { return msgs.length; }) ? flashMessages : false,
		lists: lists,
		userModel: keystone.get('user model'),
		user: req.user,
		role: (req.user && keystone.get('admin access control')) ? req.user[keystone.get('admin access control')] : null,
		title: 'Keystone',
